...
public class MultiDataSourceRegister implements EnvironmentAware, ImportBeanDefinitionRegistrar {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        String typeStr = evn.getProperty("spring.datasource.type",DATASOURCE_TYPE_DEFAULT); //默认数据源类型
        ...
        Map custom = binder.bind("spring.datasource.multi", Map.class).get(); //获取其他数据源配置
        DataSourceContextHandler.dataSourceIds.add("dataSource");

        Iterator entries = custom.entrySet().iterator();
        while (entries.hasNext()) {
            Map.Entry entry = (Map.Entry) entries.next();
            String key = String.valueOf(entry.getKey());
            config = (Map) entry.getValue();
            clazz = getDataSourceType(config.get("type")==null ? DATASOURCE_TYPE_DEFAULT : (String) config.get("type"));
            ...
            sourceMap.put(key, consumerDatasource); //获取数据源的key，以便通过该key可以定位到数据源
            DataSourceContextHandler.dataSourceIds.add(key);
        }
...
//        }
        ...
        define.setBeanClass(DataSourceRouting.class); //设置bean的类型，此处MultiDataSource是继承AbstractRoutingDataSource的实现类
